...
if (_conType in ["bunker","wall"]) then {
	...
	*/
	...
	while {alive _baseAnchor && time < (_time + TIME_BEFORE_DELETE) && !_complete} do {
		...
		if (floor time % 10 == 0) then
		{
			...
			if (_nearbyMen >= REQUIRE_MEMBERS && count _nearbyCrates > 0) then {
				...
				{
					[_prop getVariable ["MCC_loadedBy",""], 200 ,"For logstics"] remoteExec ["MCC_fnc_addRating", 0];
				};
				...
				//Update marker
				compile format ['deleteMarker "%1";',_markerName] remoteExec ["BIS_fnc_spawn", _side, false];
				sleep 1;
	...
	sleep 2;
	//Update marker
	compile format ['deleteMarker %1;',str _markerName] remoteExec ["BIS_fnc_spawn", _side, false];
	//Clear stuff
	{
		deletevehicle _x;
		sleep 1;
