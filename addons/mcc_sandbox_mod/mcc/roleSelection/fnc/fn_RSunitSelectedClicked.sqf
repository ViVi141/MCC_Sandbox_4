...
switch (toLower _data) do {
    case "kick": {
    	if (player == leader _unit && _unit != player) then {
			...
			[_str,0,0.2,5,1,0.0] remoteExec ["bis_fnc_dynamictext", group _unit, false];
			[_unit] join grpNull;

            if ((missionNamespace getVariable ["CP_activated",false]) && isPlayer _unit) then {
                [] remoteExec ["MCC_fnc_setGear", _unit, false];
            };
		};
    };
    ...
    case "commander": {
    	if (player == leader _unit && _unit != player) then {
    		[[_unit],{(group (_this select 0)) selectLeader (_this select 0);}] remoteExec ["bis_fnc_spawn", group _unit, false];
    	};
    };
