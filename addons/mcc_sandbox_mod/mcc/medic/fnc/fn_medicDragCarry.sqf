...
if (_carry) then
{
	...
	[[_targetUnit],{(_this select 0) playactionNow "grabCarried"}] remoteExec ["BIS_fnc_spawn", _targetUnit, false];
...
[[_targetUnit],{(_this select 0) playactionNow "released"}] remoteExec ["BIS_fnc_spawn", _targetUnit, false];
if (_targetUnit getVariable ["MCC_medicUnconscious",false]) then
{
	[[_targetUnit],{(_this select 0) playactionNow "Unconscious"}] remoteExec ["BIS_fnc_spawn", _targetUnit, false];
};
