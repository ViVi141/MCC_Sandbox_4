...
if (_available) then {
	...
	//================================================== Squad leader params ========================================================================//
	if (isNil "MCC_initConstract_bunker") then {MCC_initConstract_bunker = ["B_officer_F","O_officer_F","I_officer_F","C_Nikos"]};

	//If we want precise placing
	private ["_action","_startPos","_playerSideID"];

	_startPos = getPos player;
	_playerSideID = (playerSide call BIS_fnc_sideID) min 3;
	_vehicleType = switch (_conType) do {
						case "bunker":	{(missionNamespace getvariable ["MCC_initConstract_bunker",["Land_BagBunker_Small_F","Land_BagBunker_Small_F","Land_BagBunker_Small_F","Land_BagBunker_Small_F"]]) select _playerSideID};
						case "wall":	{(missionNamespace getvariable ["MCC_initConstract_wall",["Land_BagFence_Long_F","Land_HBarrier_3_F","Land_BagFence_Long_F","Land_BagFence_Long_F"]]) select _playerSideID};
						case "hmg":	{(missionNamespace getvariable ["MCC_initConstract_hmg",["O_HMG_01_F","B_HMG_01_F","O_HMG_01_F","O_HMG_01_F"]]) select _playerSideID};
						case "hmgh":	{(missionNamespace getvariable ["MCC_initConstract_hmgh",["O_HMG_01_F","B_HMG_01_high_F","O_HMG_01_F","O_HMG_01_F"]]) select _playerSideID};
						case "gmg":	{(missionNamespace getvariable ["MCC_initConstract_gmg",["O_GMG_01_F","B_GMG_01_F","O_GMG_01_F","O_GMG_01_F"]]) select _playerSideID};
						case "gmgh":	{(missionNamespace getvariable ["MCC_initConstract_gmgh",["O_GMG_01_high_F","B_GMG_01_high_F","O_GMG_01_high_F","O_GMG_01_high_F"]]) select _playerSideID};
						case "at":	{(missionNamespace getvariable ["MCC_initConstract_at",["O_static_AT_F","B_static_AT_F","O_static_AT_F","O_static_AT_F"]]) select _playerSideID};
						case "aa":	{(missionNamespace getvariable ["MCC_initConstract_aa",["O_static_AA_F","B_static_AA_F","O_static_AA_F","O_static_AA_F"]]) select _playerSideID};
						case "mortar":	{(missionNamespace getvariable ["MCC_initConstract_mortar",["O_Mortar_01_F","B_Mortar_01_F","O_Mortar_01_F","O_Mortar_01_F"]]) select _playerSideID};
