...
//----------------------------Presets---------------------------------------------------------
mccPresetsVehicle = [
					 [(localize "STR_MCC_PRESET_SET_EMPTY_FUEL"), '_this setfuel 0;']
					,[(localize "STR_MCC_PRESET_SET_EMPTY_AMMO"), '_this setvehicleammo 0;']
					,[(localize "STR_MCC_PRESET_SET_EMPTY_CARGO"), 'clearMagazineCargoGlobal _this; clearWeaponCargoGlobal _this; clearItemCargoGlobal _this;']
					,[(localize "STR_MCC_PRESET_SET_LOCKED"), '_this setVehicleLock "LOCKED";']
					,[(localize "STR_MCC_PRESET_CLEAR_CARGO"), 'clearMagazineCargo _this; clearWeaponCargo _this; clearItemCargo _this;']
					,[(localize "STR_MCC_PRESET_ADD_CREW_UAV"),'if (isServer) then {createVehicleCrew _this;group _this setvariable ["MCC_canbecontrolled",true,true];};']
					,[(localize "STR_MCC_PRESET_ADD_CARGO_UNITS"), 'if (isServer) then {[_this] call MCC_fnc_populateVehicle};']
					,[(localize "STR_MCC_PRESET_ECM_CAN_JAMM_IED"),'if (isServer) then {_this setvariable ["MCC_ECM",true,true]};']
					,[(localize "STR_MCC_PRESET_LOGISTIC_VEHICLE_CREATE_FOB"),'_this addAction ["<t color=""#99FF00"">Create FOB </t>", "'+MCC_path+'mcc\roleSelection\scripts\createFOB.sqf",[],6,false, false,"teamSwitch","(driver vehicle _target == _this) && (speed (vehicle _target) == 0)"];']
					,[(localize "STR_MCC_PRESET_DISABLE_SIMULATION"),'_this enableSimulation false;']
					,[(localize "STR_MCC_PRESET_CAN_BE_CONTROLED_USING_MCC_CONSOLE"), '(group _this) setvariable ["MCC_canbecontrolled",true,true];']
					,[(localize "STR_MCC_PRESET_RECRUITABLE"), '_this addAction [format ["Recruit %1", name _this], "'+MCC_path+'mcc\general_scripts\hostages\hostage.sqf",[2],6,false,true];']
					,[(localize "STR_MCC_PRESET_JOIN_PLAYER"), '[_this] join (group player);']
					,[(localize "STR_MCC_PRESET_SET_RENEGADE"), '_this addrating -2001;']
					,[(localize "STR_MCC_PRESET_SET_MOBILE_RESPAWN_WEST"),'[west,_this] call BIS_fnc_addRespawnPosition;']
					,[(localize "STR_MCC_PRESET_SET_MOBILE_RESPAWN_EAST"),'[east,_this] call BIS_fnc_addRespawnPosition;']
					,[(localize "STR_MCC_PRESET_SET_MOBILE_RESPAWN_GUER"),'[resistance,_this] call BIS_fnc_addRespawnPosition;']
					,['', '']
					,[(localize "STR_MCC_PRESET_ARTILLERY"),'']
					,[(localize "STR_MCC_PRESET_AMBIENT_ARTILLERY_CANNON"), '[0,_this] spawn MCC_fnc_amb_Art;']
					,[(localize "STR_MCC_PRESET_AMBIENT_ARTILLERY_ROCKETS"), '[1,_this] spawn MCC_fnc_amb_Art;']
					,[(localize "STR_MCC_PRESET_AMBIENT_AA_CANNON_ROCKETS"), '[2,_this] spawn MCC_fnc_amb_Art;']
					,[(localize "STR_MCC_PRESET_AMBIENT_AA_SEARCH_LIGHT"), '[3,_this] spawn MCC_fnc_amb_Art;']
					,[(localize "STR_MCC_PRESET_GENERAL"),'']
					,[(localize "STR_MCC_PRESET_DESTROY_VEHICLES"), '_this setdamage 1;']
					,[(localize "STR_MCC_PRESET_FLIP_VEHICLES"), '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,[(localize "STR_MCC_PRESET_VIRTUAL_ARSENAL_BIS"), 'if (isServer) then {["AmmoboxInit",[_this,true]] call BIS_fnc_arsenal};']
					,[(localize "STR_MCC_PRESET_DESTROYABLE_BY_SATCHELS_ONLY"), '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,[(localize "STR_MCC_PRESET_GOD_MOD"), '_this allowDamage false;']
					,[(localize "STR_MCC_PRESET_AIRCRAFT_CARRIER_CONSOLE"), '[_this,"Aircraft Carrier Console","","","(alive _target) && (_target distance _this < 5)","(alive _target) && (_target distance _this < 5)",{},{},{[] spawn MCC_fnc_LHDspawnMenuInit},{},[],3,10,false,false] call bis_fnc_holdActionAdd;']
					,['', '']
					,[(localize "STR_MCC_PRESET_EFFECTS"),'']
					,[(localize "STR_MCC_PRESET_SANDSTORM"),'[_this] call BIS_fnc_sandstorm;']
					,[(localize "STR_MCC_PRESET_FLIES"),'[getposatl _this] call BIS_fnc_flies;']
					,[(localize "STR_MCC_PRESET_SMOKE"),'if (isServer) then {_effect = "test_EmptyObjectForSmoke" createVehicle (getpos _this);_effect setpos (getpos _this)};']
					,[(localize "STR_MCC_PRESET_FIRE"),'if (isServer) then {_effect = "test_EmptyObjectForFireBig" createVehicle (getpos _this);_effect setpos (getpos _this)};']
					,['', '']
					,[(localize "STR_MCC_PRESET_MISC"),'']
					,[(localize "STR_MCC_PRESET_CREATE_LOCAL_MARKER"), '_this execVM "'+MCC_path+'mcc\general_scripts\create_local_marker.sqf";']
					];

mccPresetsUnits = [
					 [(localize "STR_MCC_PRESET_RECRUITABLE"), '_this addAction [format ["Recruit %1", name _this], "'+MCC_path+'mcc\general_scripts\hostages\hostage.sqf",[2],6,false,true];']
					,[(localize "STR_MCC_PRESET_MAKE_HOSTAGE"), '_this execVM "'+MCC_path+'mcc\general_scripts\hostages\create_hostage.sqf";']
					,[(localize "STR_MCC_PRESET_JOIN_PLAYER"), '[_this] join (group player);']
					,[(localize "STR_MCC_PRESET_SET_RENEGADE"), '_this addrating -2001;']
					,[(localize "STR_MCC_PRESET_STAND_UP"), '_this setUnitPos "UP";']
					,[(localize "STR_MCC_PRESET_KNEEL"), '_this setUnitPos "Middle";']
					,[(localize "STR_MCC_PRESET_PRONE"), '_this setUnitPos "DOWN";']
					,[(localize "STR_MCC_PRESET_REMOVE_ALL_WEAPONS"), 'removeAllWeapons _this;']
					,[(localize "STR_MCC_PRESET_REMOVE_ALL_ITEMS"), 'removeAllItems _this;']
					,[(localize "STR_MCC_PRESET_UNCONSCIOUS_MCC_MEDIC_SYSTEM"), '[_this,_this] spawn MCC_fnc_unconscious']
					,[(localize "STR_MCC_PRESET_CAN_BE_CONTROLED_USING_MCC_CONSOLE"), '(group _this) setvariable ["MCC_canbecontrolled",true,true];']
					,[(localize "STR_MCC_PRESET_GOD_MOD"), '_this allowDamage false;']
					,['', '']
					,[(localize "STR_MCC_PRESET_GENERAL"),'']
					,[(localize "STR_MCC_PRESET_KILL_UNIT"), '_this setdamage 1;']
					,[(localize "STR_MCC_PRESET_FLIP_UNIT"), '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,[(localize "STR_MCC_PRESET_KILLE_BY_SATCHELS_ONLY"), '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,['', '']
					,[(localize "STR_MCC_PRESET_EFFECTS"),'']
					,[(localize "STR_MCC_PRESET_SANDSTORM"),'[_this] call BIS_fnc_sandstorm;']
					,[(localize "STR_MCC_PRESET_FLIES"),'[getposatl _this] call BIS_fnc_flies;']
...
mccPresetsObjects = [
					 [(localize "STR_MCC_PRESET_PICKABLE_OBJECT"),'_this call MCC_fnc_pickItem;']
					,[(localize "STR_MCC_PRESET_DISABLE_SIMULATION"),'_this enableSimulation false;']
					,[(localize "STR_MCC_PRESET_DESTROY_OBJECT"), '_this setdamage 1;']
					,[(localize "STR_MCC_PRESET_FLIP_OBJECT"), '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,[(localize "STR_MCC_PRESET_VIRTUAL_ARSENAL_BIS"), '["AmmoboxInit",[_this,true]] call BIS_fnc_arsenal']
					,[(localize "STR_MCC_PRESET_DESTROYABLE_BY_SATCHELS_ONLY"), '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,[(localize "STR_MCC_PRESET_GOD_MOD"), '_this allowDamage false;']
					,[(localize "STR_MCC_PRESET_AIRCRAFT_CARRIER_CONSOLE"), '[_this,"Aircraft Carrier Console","","","(alive _target) && (_target distance _this < 5)","(alive _target) && (_target distance _this < 5)",{},{},{[] spawn MCC_fnc_LHDspawnMenuInit},{},[],3,10,false,false] call bis_fnc_holdActionAdd;']
					,['', '']
