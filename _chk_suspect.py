...
# check the two suspect files
for rel in [r"mcc\dialogs\mcc_boxGen_change.sqf", r"mcc\pop_menu\spawn_group3d.sqf", r"mcc\missionWizard\fnc\fn_MWSpawnInZone.sqf", r"mcc\roleSelection\fnc\fn_buildSpawnPoint.sqf"]:
    p = r"addons\mcc_sandbox_mod" + '\\' + rel
    with open(p, 'r', encoding='utf-8', errors='replace') as f:
        content = f.read()
    iss = check(content)
    print(rel.split('\\')[-1], "->", "CLEAN" if not iss else str(iss[:6]))
